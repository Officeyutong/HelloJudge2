...
@router.route("/forget_username", methods=["POST"])
@unpack_argument
def api_user_find_username_by_phone(phone: str, code: str):
    if not config.USE_PHONE_WHEN_REGISTER_AND_RESETPASSWD:
        return make_response(-1, message="不使用手机号注册用户")
    user: User = db.session.query(User.username).filter(
        User.phone_number == phone).one_or_none()
    if not user:
        return make_response(-1, message="该手机号未注册!")
    if not phoneauth.validate_sms_code(phone, code):
        return make_response(-1, message="验证码校验失败")
    return make_response(0, data={
