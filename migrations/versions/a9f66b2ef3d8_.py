...
Revision ID: a9f66b2ef3d8
Revises: 9ea9505cdfc7
Create Date: 2019-12-26 22:19:26.505247
...
revision = 'a9f66b2ef3d8'
down_revision = '9ea9505cdfc7'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    op.alter_column('discussions', 'top',
               ...
               type_=sa.Boolean(),
               existing_nullable=False)
    op.add_column('problem_set', sa.Column('description', sa.Text(), nullable=True))
    conn: Connection = op.get_bind()
    # 默认说明文本
    conn.execute(
        """UPDATE problem_set set description = "" where 1 = 1"""
    )
...
def downgrade():
    ...
    op.alter_column('problem_set', 'private',
               ...
               type_=mysql.TINYINT(display_width=1),
               existing_nullable=False)
    op.drop_column('problem_set', 'description')
    op.alter_column('discussions', 'top',
               existing_type=sa.Boolean(),
               type_=mysql.TINYINT(display_width=1),
               existing_nullable=False)
